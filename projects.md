...
<p>1. Causal Index — compute the influence of each input pixel on each output by summing weighted paths through the network, then render as a heat map per digit.</p>
<p>2. Iterative Input Adaptation — start from random noise, apply stochastic hill climbing until the network confidently classifies the input as a target digit. A technique similar to DeepDream.</p>
